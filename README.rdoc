...
in 2nd-level hashes is one of the advantages of using this gem.
...
* Setting.key_name(:sub_key_name, :sub_sub_key_name) returns value from the 3rd level hash if available. The algorithm is recursive, so there is no limit to the number of nested hashes you can access this way.
* Special syntax Setting[:key_name], Setting[:key_name][:sub_key_name], etc also supported. This method, however, does not  
